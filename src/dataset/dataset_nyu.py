...
@dataclass
class DatasetNYUCfg(DatasetCfgCommon):
    name: Literal["nyu"]
    roots: list[Path]
...
class NYUDepthV2Dataset(IterableDataset):
    cfg: DatasetNYUCfg
    stage: Stage
    ...
    chunks: list[Path]
    near: float = 0.1
    far: float = 10.0  # NYU数据集的深度范围通常较小
    ...
    def download_dataset(self):
        # 实现数据集下载逻辑
        # 这里只是一个示例，实际实现需要根据NYU Depth V2数据集的下载方式进行调整
        for root in self.cfg.roots:
            root.mkdir(parents=True, exist_ok=True)
            # 检查数据集是否已下载
            if not (root / "nyu_data.torch").exists():
                print(f"Downloading NYU Depth V2 dataset to {root}")
                # 这里应该添加实际的下载代码
                # 例如使用wget或requests下载数据集
                # 然后解压并处理成torch格式
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for run_idx in range(int(times_per_scene * len(chunk))):
                ...
                # 跳过形状不正确的图像
                context_image_invalid = context_images.shape[1:] != (3, 480, 640)  # NYU的图像尺寸通常是480x640
